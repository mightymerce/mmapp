    ...
    function ($http) {
      return {
        ...
        updateOrder: function updateOrder(id,order) {
          console.log('Order Status from input: ' +order.orderStatus);
          console.log('Order Mail Message: ' +order.ordereMailCustomerShipMessage);
          console.log('Order Tracking No.: ' +order.orderTrackingNo);
          var promise = $http({
            method: 'PUT',
            url: 'api/orders/' +id,
            params: {
            ...
            console.log('Order Status after update to shipped:' +response.data.orderStatus);
            // The return value gets picked up by the then in the controller.
            return response.data;
          }, function errorCallback(response) {
            // called asynchronously if an error occurs
